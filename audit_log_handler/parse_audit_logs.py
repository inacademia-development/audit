...
BASE_DIR = dirname(realpath(__file__)) + "/input"
LOGS_DIR = BASE_DIR + "/remote"
IDPS_DIR = BASE_DIR + "/idps"
IDPS_FILE = IDPS_DIR + "/entitiyids_plain.json"
...
if len(sys.argv) < 5:
    sys.exit("Need MYSQL_HOST MYSQL_USER MYSQL_PWDMYSQL_DB MYSQL_DB")
...
def write_logs_sql(cur, audit_dict):
    ...
    query = ("INSERT INTO `inac_auditlog`.`logs` ("
...
def write_idps_sql(cur, key, name, domain, country):
    # Create a Cursor object
    #cur = db.cursor()
    ...
    name = name.replace("'", "\\'")
    query = ("INSERT INTO `inac_auditlog`.`idps` "
             " ( `idp_entityid`, `idp_displayname`, `idp_domain`, `idp_country`)  "
             " VALUES "
             " ( '"  + key + "',"
             "   '"  + name + "',"
             "   '"  +  domain  + "',"
...
def write_client_sql(cur, key, name):
    ...
    query = ("INSERT INTO `inac_auditlog`.`clients` "
...
if UPDATE_IDPS:
    idps_dict = parse_idps_file(IDPS_FILE, "")
