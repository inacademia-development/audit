...
if [ ! -d "$REMOTE_DIR" ]; then
	...
	GIT_SSH_COMMAND='ssh -i /home/niels/dev/InAcademia/inacademia-audit/audit_log_handler_docker/config/id_rsa_inacademia' /usr/bin/git clone git@gitlab.geant.org:inacademia/audit_logs.git $REMOTE_DIR
else
	...
	cd $REMOTE_DIR
	GIT_SSH_COMMAND='ssh -i /home/niels/dev/InAcademia/inacademia-audit/audit_log_handler_docker/config/id_rsa_inacademia' /usr/bin/git pull git@gitlab.geant.org:inacademia/audit_logs.git
fi
...
if [ ! -d "$ADMIN_DATA_DIR" ]; then
	...
	GIT_SSH_COMMAND='ssh -i /home/niels/dev/InAcademia/inacademia-audit/audit_log_handler_docker/config/id_rsa_inacademia' /usr/bin/git clone git@gitlab.geant.org:inacademia/admin_data.git $ADMIN_DATA_DIR
else
	# Remote dir exists, so cd into the remote dir and pull the lates version of the logspull a fresh copy of tha audot logs into the remote dir
	echo "$ADMIN_DATA_DIR found, updating audit logs"
	cd $ADMIN_DATA_DIR
	GIT_SSH_COMMAND='ssh -i /home/niels/dev/InAcademia/inacademia-audit/audit_log_handler_docker/config/id_rsa_inacademia' /usr/bin/git pull git@gitlab.geant.org:inacademia/admin_data.git
fi
...
if [ ! -d "$IDP_HINT_DIR" ]; then
	...
	GIT_SSH_COMMAND='ssh -i /home/niels/dev/InAcademia/inacademia-audit/audit_log_handler_docker/config/id_rsa_inacademia' /usr/bin/git clone https://github.com/InAcademia/idp_hint.git $IDP_HINT_DIR
else
	# Remote dir exists, so cd into the remote dir and pull the lates version of the logspull a fresh copy of tha audot logs into the remote dir
	echo "$IDP_HINT_DIR found, updating idp hint data"
	cd $IDP_HINT_DIR
	GIT_SSH_COMMAND='ssh -i /home/niels/dev/InAcademia/inacademia-audit/audit_log_handler_docker/config/id_rsa_inacademia' /usr/bin/git pull https://github.com/InAcademia/idp_hint.git
fi
